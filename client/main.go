...
func main() {
	...
	conn, err := setupGrpcConnection(serverAddr, tlsCertFile)
	if err != nil {
		log.Fatal(err)
	}
	defer conn.Close()
	...
	case "GetUser":
		...
		result, err := c.GetUser(context.Background(), &svc.UserGetRequest{
			Email: "panic@doe.com", // @前为panic时触发服务端panic，其它正常
		})
...
func setupGrpcConnection(addr, tlsCertFile string) (*grpc.ClientConn, error) {
